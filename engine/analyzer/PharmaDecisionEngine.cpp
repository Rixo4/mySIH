// PharmaDecisionEngine.cpp  —  REFACTORED
//
// SCIENTIFIC AUDIT SUMMARY
// ════════════════════════════════════════════════════════════════════════════
// This refactor eliminates every violation of the causal pipeline rule:
//
//   Channel Blockade → Single-Neuron Dynamics → Network Dynamics
//       → Measured Metrics → Biological Interpretation
//       → Mechanistic Evidence → Per-Dose Classification
//       → Dose-Response Analysis → Dominant Mechanism
//       → Safety Analysis → Confidence Analysis
//       → Ontology Summary → Final Report
//
// VIOLATIONS FOUND AND CORRECTED
// ────────────────────────────────────────────────────────────────────────────
// [V1]  kMeaningfulKBlockThreshold (line 14) — a channel block threshold used
//       as a primary classification gate.  REMOVED.
//
// [V2]  detectBiologicalState() line 302 — `obs.blockK >= threshold` causes an
//       early RETURN of Hyperexcitability before any biological evidence is
//       examined.  This is the single most dangerous violation: a drug that
//       blocks K+ channels is automatically labelled hyperexcitable regardless
//       of whether firing rate, NII, or seizure probability ever increased.
//       REMOVED.  Hyperexcitability now emerges from rate/NII/seizure changes.
//
// [V3]  detectBiologicalState() line 330 — `obs.blockCa >= threshold` is a
//       prerequisite for sawNetworkStabilization.  A drug that reduces
//       synchronisation without Ca blockade would never be classified as
//       stabilising.  REMOVED.  Stabilisation now emerges from sync/NII/
//       seizure reductions alone.
//
// [V4]  Per-dose loop line 825-857 — `meaningfulCaBlock` gates the
//       calciumStabilization flag, which determines whether the dose is
//       classified BiologicalState::NetworkStabilization.  Classification is
//       therefore derived from the channel, not from biology.  REMOVED.
//
// [V5]  Per-dose loop line 834-847 — `meaningfulKBlock` is the PRIMARY
//       condition for Hyperexcitability, checked BEFORE the biological
//       evidence conditions.  REMOVED.
//
// [V6]  Per-dose loop lines 857-863 — a second `meaningfulKBlock` guard inside
//       the NetworkStabilization branch can flip a stabilising dose to
//       Hyperexcitability purely because blockK ≥ 0.15, even when all
//       biological metrics indicate stabilisation.  REMOVED.
//
// [V7]  stabilizingResponseObserved (line 1270-1273) — requires
//       (sawMeaningfulCaBlock || finalMeaningfulCaBlock) as a hard gate.
//       A drug that stabilises the network without Ca blockade would never
//       enter the STABILIZING_RESPONSE pathway.  REMOVED.
//
// [V8]  report.meaningfulCaBlock (line 1253) — exported to the report and
//       used downstream by the Python backend and frontend.  Renamed to
//       report.stabilizationConfirmedByMetrics and computed purely from
//       biological metric reductions.  The old calciumEffectMagnitude field
//       is retained for backward compatibility but is now derived from
//       metric magnitudes, not from Ca block depth.
//
// [V9]  primaryChangeTextForState() lines 503-514 — hardcoded causal
//       narratives ("Strong sodium-channel block caused neural silencing",
//       "Calcium-channel blockade reduced synchronization") are scientifically
//       incorrect: they reverse the causal arrow (channel → biology) and
//       constitute hardcoded biological assumptions.  REPLACED with metric-
//       driven narrative generation.
//
// [V10] safetyInterpretationForState() line 533 — same Ca-channel narrative
//       hardcoded into the safety interpretation.  REPLACED.
//
// [V11] generateReasonForState() lines 561/567/580 — same Na/Ca-channel
//       narratives hardcoded into the reason field.  REPLACED.
//
// [V12] evaluate() line 1432 — recommendation reason "Calcium-channel blockade
//       reduced synchronization…" hardcoded.  REPLACED.
//
// [V13] evaluate() line 1452 — recommendation reason "Controlled sodium-channel
//       suppression observed…" hardcoded.  REPLACED.
//
// WHAT CHANNEL CONTRIBUTIONS ARE USED FOR (correct usage, retained)
// ─────────────────────────────────────────────────────────────────
// blockNa, blockK, blockCa remain in DoseObservation as observational data.
// They are NEVER read inside this file.  They may be annotated in a separate
// MechanisticAnnotation layer (not this engine) as supportive explanatory
