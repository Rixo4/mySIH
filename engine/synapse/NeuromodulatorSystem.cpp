...
NeuromodulatorGainModifiers computeNeuromodulatorGainModifiers(
    ...
    // D1: shrinks adaptation current, boosts NMDA. Both float toward their
    // ceilings as D1 occupancy rises; ceiling=default (0 reduction / 1.0x
    // gain) means this specific lever stays exactly inert even if the drug
    // configures the OTHER D1 lever but not this one.
    {
        const float occ = neuromodulatorOccupancy(dose, profile.d1.ec50, profile.d1.hill);
    ...
    {
        const float occ = neuromodulatorOccupancy(dose, profile.d2.ec50, profile.d2.hill);
    ...
    {
        const float occ = neuromodulatorOccupancy(dose, profile.ht1a.ec50, profile.ht1a.hill);
    ...
    // (reduced IAHP) -- both multiply into the same running scale factors
    // as D1's adaptation lever and 5-HT1A's gK lever above, so a drug
    // hitting multiple receptors composes correctly (each contributes its
    // own multiplicative factor).
    {
        const float occ = neuromodulatorOccupancy(dose, profile.ht2a.ec50, profile.ht2a.hill);
